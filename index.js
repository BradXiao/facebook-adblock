...
// @author       Placidina
// @supportURL   https://github.com/Placidina/facebook-adblock
...
(function () {
    ...
    var searchedNodes = [{
        ...
        'content': {
            ...
            'pl': ['Sponsorowane'],
            'ps': ['تمويل شوي'],
            'pt': ['Patrocinado', 'PaPtroPcinPadoP'],
            'ru': ['Реклама'],
            'sa': ['प्रायोजितः |'],
    ...
    function preCompare(node) {

        let toCompare = node[nodeContentKey];
        if (node.children.length) {

            toCompare = '';
            for (let childIterator = 0; childIterator < node.children.length; childIterator++) {

                let fontSize = parseInt(window.getComputedStyle(node.children[childIterator], null).fontSize, 10);
                if (fontSize > 0) {
    ...
    function isSponsored(story) {
        ...
        for (let typeIterator = 0; typeIterator < searchedNodes.length; typeIterator++) {

            for (let selectorIterator = 0; selectorIterator < searchedNodes[typeIterator].selector.length; selectorIterator++) {

                let nodes = story.querySelectorAll(searchedNodes[typeIterator].selector[selectorIterator]);
                for (let nodeIterator = 0; nodeIterator < nodes.length; nodeIterator++) {

                    let nodeContent = nodes[nodeIterator][nodeContentKey];
                    if (nodeContent) {

                        for (let targetIterator = 0; targetIterator < searchedNodes[typeIterator].content.length; targetIterator++) {

                            if (exclude(searchedNodes[typeIterator]) && exclude(nodes[nodeIterator])) {
                                continue;
                            }

                            if (nodes[nodeIterator].offsetParent == null) {
                                continue;
                            }

                            if (searchedNodes[typeIterator].preCompare) {
                                comparisonString = preCompare(nodes[nodeIterator]);
                            } else {
    ...
    function process() {
        ...
        let stories;
        for (let i = 0; i < storySelectors.length; i++) {
